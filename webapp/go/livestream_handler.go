...
func reserveLivestreamHandler(c echo.Context) error {
	...
	if err := tx.SelectContext(ctx, &slots, "SELECT slot, start_at, end_at FROM reservation_slots WHERE start_at >= ? AND end_at <= ? FOR UPDATE", req.StartAt, req.EndAt); err != nil {
